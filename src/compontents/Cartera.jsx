...
export default function Cartera({ transactions, id, balance, cryptoData, total, coins, name, isMobile }) {
  ...
  const everyImg = coinsImgs.map((valor, indx) => {
    if (indx < 4) {
      return (
        <div
          key={indx}
          className="bg-[#6c7b904d] transition-all shadow-md hover:ring-2  ring-[#6c7b904d] h-8 w-8 rounded-full flex justify-center items-center"
        >
          <img className="rounded-full" src={valor}></img>
        </div>
      );
    }
    return;
      ...
      {isMobile && (
        <motion.div>
          <div id={id} className=" cartera flex flex-col items-center justify-center gap-3 relative bord rounded-lg bg-[#3F3E45]  text-white text-sm p-5 ">
            ...
            {toggleEdit && (
              <>
                <div className=" absolute top-2  w-[260px] h-12 border-2 border-[#faa018] rounded-full py-1 px-4 flex justify-center items-center gap-2 mx-5">
                  <input
